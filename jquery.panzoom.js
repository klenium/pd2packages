...
(function(global, factory) {
	...
	Panzoom.prototype = {
		...
		setMatrix: function(matrix, options) {
			...
			var sw = 0, sh = 0, dims, container, marginW, marginH, diffW, diffH, left, top, width, height;
			var scale = +matrix[0];
			var $parent = this.$parent;
			...
			if (contain) {
				...
				if (contain === 'invert') {
					...
					if (width > container.width)
						...
						sw = (container.width - width) / 2;
					if (height > container.height)
						marginH += (container.height - height) / 2;
					else
						sh = (container.height - height) / 2;
					matrix[4] = sw+Math.max(Math.min(matrix[4], marginW - left), -marginW - left - diffW);
					matrix[5] = sh+Math.max(Math.min(matrix[5], marginH - top), -marginH - top - diffH + dims.heightBorder);
				} else {
